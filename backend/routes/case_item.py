...
def list_items(
    ...
    try:
        return case_item_service.list_case_items(
            db, case_id, user_id=current_user.id
        )
...
def create_item(
    ...
    try:
        return case_item_service.create_case_item(
            db, case_id, item, user_id=current_user.id
        )
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def read_item(
    ...
    db_item = case_item_service.get_case_item_by_id(
        db, case_id, item_id, user_id=current_user.id
    )
    if db_item is None:
...
def update_item(
    ...
    try:
        updated_item = case_item_service.update_case_item(
            db, case_id, item_id, item, user_id=current_user.id
        )
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def delete_item(
    ...
    try:
        deleted = case_item_service.delete_case_item(
            db, case_id, item_id, user_id=current_user.id
        )
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
